"""Bounded C API CPU diagnostic using the frozen Issue 107 business styles.
...
p.add_argument('--query-ids', action='store_true', help='Query every quantity id after replacement, before layout, like the business page')
...
for count in (200, 400, 800):
    # A zeroed callback table uses the C API's deterministic fallback font metrics.
    callbacks = (ptr * 64)()
    ...
    for iteration in range(12):
        ...
        html = ''.join(f'<div class="row" id="item-{i}"><img src="../fixtures/image_fixture.png"><span class="name">精密组件 Precision component {i:04}</span><span class="state">待处理 Pending</span><span class="qty"{" id=qty-"+str(i) if args.query_ids else ""}>{20+i%73}</span></div>' for i in order).encode()
        reset(service)
        ...
        if args.query_ids:
            for i in range(count):
                node = get(service, f'qty-{i}'.encode())
                assert node
                release(node)
        ...
        if iteration >= 2:
            samples.append(dict(parseMs=(parsed-start)/1e6, lookupMs=(queried-parsed)/1e6, renderMs=(end-queried)/1e6,
                                styleMs=sum(values[6:9])/1e6, matchedNodes=sum(values[3:6])))
    geometry = []
    for i in range(count):
        node = get(service, f'item-{i}'.encode())
        rect = (c.c_float * 4)()
...
args.output.write_text(json.dumps(dict(dll=str(args.dll.resolve()), dllSha256=hashlib.sha256(args.dll.read_bytes()).hexdigest(), fixtureSha256=hashlib.sha256(source.encode()).hexdigest(), queryIds=args.query_ids, scriptSha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(), results=results), indent=2), encoding='utf-8')
